...
class MultiHeadAttention(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        if mask is not None:
            ...
            attention_scores = attention_scores.masked_fill(mask_bool, fill_value)
    ...
    def forward(self, q, k, v, mask):
        query = self.w_q(q) #multiply query by Wq, (batch_size, seq_len, d_model) -> (batch_size, seq_len, d_model)
        key = self.w_k(k) #multiply key by Wk, (batch_size, seq_len, d_model) -> (batch_size, seq_len, d_model)
        value = self.w_v(v) #multiply value by Wv, (batch_size, seq_len, d_model) -> (batch_size, seq_len, d_model)
